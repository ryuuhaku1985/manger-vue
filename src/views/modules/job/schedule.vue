    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          <el-tag v-if="scope.row.jobType === 0" size="small">多次</el-tag>
          <el-tag v-else size="small">仅一次</el-tag>
        </template>
      </el-table-column>
      <el-table-column
        prop="remark"
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-tag v-else-if="scope.row.status === -1" size="small">结束</el-tag>
      ...
      <el-table-column
        ...
        label="操作">
        <template slot-scope="scope">
          <el-button v-if="isAuth('sys:schedule:update') && scope.row.status !== -1" type="text" size="small" @click="addOrUpdateHandle(scope.row.jobId)">修改</el-button>
          <el-button v-if="isAuth('sys:schedule:delete')" type="text" size="small" @click="deleteHandle(scope.row.jobId)">删除</el-button>
          <el-button v-if="isAuth('sys:schedule:pause') && scope.row.status !== -1" type="text" size="small" @click="pauseHandle(scope.row.jobId)">暂停</el-button>
          <el-button v-if="isAuth('sys:schedule:resume') && scope.row.status !== -1" type="text" size="small" @click="resumeHandle(scope.row.jobId)">恢复</el-button>
          <el-button v-if="isAuth('sys:schedule:run') && scope.row.status !== -1" type="text" size="small" @click="runHandle(scope.row.jobId)">立即执行</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
    ...
    methods: {
      ...
      getDataList () {
        ...
        this.$http({
          ...
          if (data && data.code === '0') {
            this.dataList = data.resp.page.list
            this.totalPage = data.resp.page.totalCount
          } else {
            this.dataList = []
            this.totalPage = 0
          }
